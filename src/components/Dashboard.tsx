...
export default function Dashboard() {
  ...
  const fetchUpcomingRenewals = async () => {
    try {
      ...
      let query = supabase
        .from('policies')
        .select('id, policy_number, end_date, policy_type, clients(full_name), insurance_companies(name)')
        .gte('end_date', today.toISOString())
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className="font-medium text-gray-900 text-sm">{renewal.clients?.full_name || 'Müşteri'}</p>
                      <p className="text-xs text-gray-600 mt-1">
                        {renewal.policy_number} • {renewal.policy_type}
                      </p>
